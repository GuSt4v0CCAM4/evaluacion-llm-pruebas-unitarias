...
def save_dataset(
    ...
    if "id" not in fields:
        raise ValueError(f"Not a proper data class: {clz}; need to have id")
...
def load_all_dataset(
    ...
    for i in range(len(ids)):
        dataset[i] = clz(id=ids[i], **{f: f2fields[f][i] for f in f2fields})
    ...
    for t in token_vals:
        n = [
            ...
            for c in re.findall(
                ...
                t.encode("ascii", errors="ignore").decode().strip(),
            )
            if len(c) > 0
        ]
...
def tokenize_code(line: str) -> List[str]:
    ...
    try:
        ...
        tokens = re.findall(
            r"[a-zA-Z0-9]+|[^\sa-zA-Z0-9]|[^_\sa-zA-Z0-9]", line.strip()
        )
    return tokens


if __name__ == "__main__":
    j_code = '@Test(expected = Exception.class)\npublic void failsToConstuctFromDifferentlySizedLists() throws Exception {\n    List<String> smallList = Arrays.asList("foo", "bar", "baz");\n    List<String> largeList = Arrays.asList("foo", "bar", "baz", "qux", "quuz");\n    try {\n        new ZippedMap<>(smallList, largeList);\n        fail("Expected exception not thrown");\n    } catch (Exception e) {\n        assertTrue(e.getMessage().contains("Lists must have the same size"));\n    }\n}'
    print(tokenize_code(j_code))
