...
import dataclasses
import enum
from typing import Callable, Dict, Iterable, List, Optional, OrderedDict, Tuple, Union
...
class AST:
    ...
    def deserialize(cls, data: list) -> "AST":
        ...
        if ":" in type_and_kind:
            ...
            ast.children = [cls.deserialize(child) for child in data[2:-1]]
            ast.raw_code = data[-1]
        return ast
    ...
    def format_code(self, with_lineno: bool = True) -> str:
        s = ""
        last_lineno = None
        ...
        while len(queue) > 0:
            ...
            if not stop_func(cur):
                if cur.children is not None:
                    if order == TraverseOrder.BREATH_FIRST:
                        queue += cur.children
    ...
    def traverse_terminal(
        self, order: TraverseOrder = TraverseOrder.DEPTH_FIRST
    ) -> Iterable["AST"]:
        """Traverse over each terminal node"""
        for node in self.traverse(order=order):
            if node.is_terminal():
                yield node

    def find_all(self, ast_type: Union[str, Tuple[str]]) -> Iterable["AST"]:
        """Find all children (including self) with given ast type(s)."""
        if isinstance(ast_type, str):
            ast_type = (ast_type,)
        return (x for x in self.traverse() if x.ast_type in ast_type)

    def find_all_d1(self, ast_type: Union[str, Tuple[str]]) -> Iterable["AST"]:
        """Find all direct children with given ast type(s)."""
        if isinstance(ast_type, str):
            ast_type = (ast_type,)
        return (x for x in self.children if x.ast_type in ast_type)

    def find_first(self, ast_type: Union[str, Tuple[str]]) -> Optional["AST"]:
        """Find first child (including self) with given ast type(s)."""
        if isinstance(ast_type, str):
            ast_type = (ast_type,)
        for x in self.traverse():
            if x.ast_type in ast_type:
                return x
        return None

    def find_first_d1(self, ast_type: Union[str, Tuple[str]]) -> Optional["AST"]:
        """Find first direct child with given ast type(s)."""
        if isinstance(ast_type, str):
            ast_type = (ast_type,)
...
class MethodBytecode:
    insns: OrderedDict[int, Insn] = dataclasses.field(
        default_factory=collections.OrderedDict
    )
    lnt: OrderedDict[int, int] = dataclasses.field(
        default_factory=collections.OrderedDict
    )
    ...
    def deserialize(cls, data: dict) -> "MethodBytecode":
        ...
        for k, v in sorted(
            ...
            lnt[k] = v
        return cls(insns=insns, lnt=lnt)

    def get_ordered_insns(self) -> List[Insn]:
        """get instructions in order"""
        return [self.insns[i] for i in self.insns.keys()]

    def get_insns_on_line(self, lineno: int) -> List[Insn]:
        insn_beg = None
        insn_end = None
        for insn_i, lineno_i in self.lnt.items():
            if insn_beg is None:
                if lineno <= lineno_i:
                    insn_beg = insn_i
            else:
        ...
        if insn_beg is None:
            raise ValueError(f"lineno {lineno} is out of bound of line number table")

        insns = []
        for insn_i, insn in self.insns.items():
            if insn_end is not None and insn_i > insn_end:
                break
            if insn_i >= insn_beg:
                insns.append(insn)
